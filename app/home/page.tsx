...
const page = () => {
  ...
  const { data: recipeData, loading: recipeLoading, refetch: refetchRecipes } = useQuery(GET_ALL_RECIPES)
  ...
  useEffect(() => {
    if (session?.user?.email !== undefined && status === "authenticated" && recipeLoading === false) {
      getAllRecipes()
